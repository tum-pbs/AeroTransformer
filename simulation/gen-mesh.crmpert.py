...
def scaling_tip(xx1, zz1, wingCoef, rdx=None, rdz=None, tip_twist=None):
    '''
...
def surface_meshing(wingCoefs: dict, folder='.', control_points: list = None):
    ...
    kink = half_span * control_points[2]
    cabin = half_span * cabin_ratio
    
    _control_points = [half_span * eta for eta in control_points]
    ...
    # chord
    chord_tip   = wingCoefs["chords"][-1]
    chord_kink  = wingCoefs["chords"][2]
    chord_cabin = wingCoefs["chords"][0]
    ...
    # twist
    tws_cs = CubicSpline(_control_points, [reduce(lambda x, y: x + y, wingCoefs['twists'][:i+1]) + twists0 for i in range(len(wingCoefs['twists']))], extrapolate=True)
    tws = tws_cs(yLEs)
    print(tws)
    # dihedral
    zLE_cs = CubicSpline([0] + _control_points, wingCoefs['DAz'], extrapolate=False)
    ...
    for i in range(len(_control_points)):
        xx1, yus, yls, _, _ = cst_foil(nn=nn0, x=np.array(xx0), cst_u=wingCoefs['cst_u'][i], cst_l=wingCoefs['cst_l'][i], t=None, tail=0.008)
    ...
    for yy in yLEs:
        if yy < _control_points[0]:
            ...
            print(f'[warning] extrapolate for y = {yy:.2f}')
    ...
    print(np.isnan(xxs1).any(), np.isnan(zzs1).any())
...
if __name__ == "__main__":
    ...
    conds = {"M3": 0.85, "AA": 2.1837, "re": 20000000.0, "temp": 300}
    
    if comm.rank == 0:
        wingCoefs = surface_meshing(wingCoefs, control_points=control_points)
    
    ds = cal_ds(Uinf=np.mean(conds['M3']) * (1.4 * 287 * conds['temp'])**0.5, cri_yplus=1.0, rhoinf=1.225, Rex=conds['re'])
    mesh_dict = volume_meshing(wingCoefs, input="wing.xyz", ds=ds, folder='.', output_plot3d=False)
